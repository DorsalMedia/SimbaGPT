...
class Paypal {
	 ...
	 * @return	bool
	 */
	function __initialize($config = array())
	{
		#Account information
		
		$config["business"] 	= $config['business']; //Account email or id
		$config["cmd"] 			= '_cart'; //Do not modify
		$config["production"] 	= FALSE;

		#Custom variable here we send the billing code-->
		$config["custom"]	= '';
		$config["invoice"]	= $config["invoice"]; //Code to identify the bill
		...
		$config["return"] 			= $config["return"]; //The URL to which PayPal redirects buyersÃ¢â‚¬â„¢ browser after they complete their payments.
		$config["cancel_return"]	= $config["cancel_return"]; //Specify a URL on your website that displays a Ã¢â‚¬Å“Payment CanceledÃ¢â‚¬Â�? page.
		$config["notify_url"] 		= $config["notify_url"];  //The URL to which PayPal posts information about the payment (IPN)
